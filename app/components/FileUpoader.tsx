...
import { cn, convertFileToBase64, convertFileToUrl, getFileType } from '@/lib/utils';
	...
	const onDrop = useCallback(
		async (acceptedFiles: File[]) => {
			...
			const uploadPromises = acceptedFiles.map(async (file: File) => {
				...
				const newFileObj = await convertFileToBase64(file);
				return await uploadFile({ file: { base64String: newFileObj, fileName: file.name, fileType: file.type }, ownerId, accountId, path }).then((uploadedFile) => {
					if (uploadedFile) {
						setFiles((prevFiles) => prevFiles.filter((f) => f.name !== file.name));
					}
				});
